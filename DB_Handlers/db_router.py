...
from DataBase import session_maker, orm_add, orm_get_card, orm_delete_card
...
db_router.callback_query.filter(AutoDeleteMessage(num_id_prew=1))
...
async def load_from_db_card(message: Message, session: AsyncSession):
    result = await orm_get_card(session, message.from_user.id)
    if result:
...
async def admin_load_all_cards(session: AsyncSession):
    string_text = ""
    for card in await orm_get_all_cards_card(session):
...
@db_router.message(IsAdmin(), IsValidNumber(), AdminStatements.id_user)
async def admin_load_one_card(message: Message, state: FSMContext, session: AsyncSession):
    ...
    for card in await orm_get_card(session, int(message.text)):
...
Предупреждений: {card.strikes}                                 
        ...
        await message.answer("Такой карточки в базе нет!", reply_markup=(inline_deleter_card(int(message.text))))
        
        
@db_router.callback_query(F.data == "delete_card")
async def admin_delete_card_from_db(update: CallbackQuery, state: FSMContext, session: AsyncSession):
    data = await state.get_data()
    await orm_delete_card(data["id_user"])
